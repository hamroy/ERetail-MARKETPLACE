...
class C_keu extends CI_Controller
{
	...
	{
		redirect('C_keu/beranda');
	}

	///rev tanggal 130717 DOMPET E-Retail	
	public function beranda()
	{
		if ($this->session->userdata('login') == TRUE and $this->session->userdata('wewenang') == 'keu') {
	...
	public function cetak($hasil_no)
	{
		if ($this->session->userdata('login') == TRUE and $this->session->userdata('wewenang') == 'keu') {
			...
			$this->load->view('keu/cetak_html', $data);
			//$this->load->view('pages/admin/beranda',$data);
		} else { ///pengan login
			$this->session->set_flashdata('pesan', 'Maaf, Anda harus login kembali .');
			redirect('Login');
	...
	{ ///cuam pemecahan
		...
		$this->session->set_flashdata('pesan', 'Data Berhasil dikosongkan' . $hasil_no);
		redirect('C_keu/#' . $hasil_no);
	}
	...
	{ ///cuam pemecahan
		...
		redirect('C_keu/cetakan_beranda/' . $hasil_no);
	}
} ///class
