<script src="<?=$this->M_setapp->static_bm()?>/plugins/jQuery/jquery-2.2.3.min.js"></script>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form id="csform<?=$gidp->id?>" role="form" action="<?=base_url('C_dompet_2/acc_pesan_voc/t/'.$gidp->id_user.'/'.$gidp->id.'/'.$id_job)?>" method="post">
  ...
  <div class="form-group">
    ...
    <input class="form-control" type="number" min="0" placeholder="Saldo" name="saldo" required />
      ...
      <div class="modal-footer">
          <input id="klikbtn<?=$gidp->id?>" value="Kirim" onclick="return confirm('Anda Yakin !!')" type="submit" class="btn btn-success btn-block" />
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
       ...
       <form role="form" action="<?=base_url('C_dompet/terima_pesan_voc/b/'.$gidp->id_user.'/'.$gidp->id.'/'.$id_job)?>" method="post" id="csform_<?=$gidp->id?>">
      <div class="modal-body">
  ...
  <div class="form-group">
    ...
    <textarea class="form-control" name="alasan"rows="3"></textarea>
      ...
      <div class="modal-footer">
       <input id="klikbtn2_<?=$gidp->id?>" value="Kirim" onclick="return confirm('Anda Yakin !!')" type="submit" class="btn btn-success btn-block" />
        <button type="button" class="btn btn-default btn-block" data-dismiss="modal">Close</button>
      </div>
     </form>
    </div>
            ...
            $(function()
            {
              $('#klikbtn2_<?=$gidp->id?>').on('click',function()
              {
                $(this).val('Sedang diproses ...')
                  .attr('disabled','disabled');
