 <section class="content-header" style="background: #ecedee;">
    ...
    <div class="well">
     <form class="form-horizontal" method="post" action="<?=base_url('C_rekapPenjualan/PostTgl/1')?>" role="form">
      <div class="row">
      <div class="col-md-3">
...
<?php
$gtmpl=$this->session->userdata('Tampil');
  ...
  <div class="well">
     <form class="form-horizontal" role="form">
        ...
        <select name="sort" class="form-control"  onchange="loadPage(this.form.elements[0])">
          ...
          <option value="<?=base_url('C_rekapPenjualan/sortStatus/0/1')?>"  <?php if($statusP=='0'){ echo 'selected';}?> >Semua</option>
          <option value="<?=base_url('C_rekapPenjualan/sortStatus/R/1')?>"  <?php if($statusP=='R'){ echo 'selected';}?> >Rekap Semua Status</option>
          <?php
          $stnam='SEMUA';
          ...
          foreach($gt_stjob->result_array() as $va){
             ...
             // echo $va['id_job'];
              ?>
                ...
                <option value="<?=base_url('C_rekapPenjualan/sortStatus/'.$va['id_job'].'/1')?>"  <?=$sl?> ><?=$va['nama_job']?></option>
      ...
      <div class="input-group" style="padding: 5px">
        <span class="input-group-addon" id="basic-addon1">PRODI</span>
                    <select name="sortprodi" class="form-control"  onchange="loadPage(this.form.elements[1])">
                      ...
                      <option value="<?=base_url('C_rekapPenjualan/sortStatusProdi/0/1')?>" <?php if($statprodi=='0'){ echo 'selected';}?> >SEMUA</option>
                      <?php
                      ...
                      foreach($gjob->result() as $jb){
                        $slpro='';
                         if($statprodi==$jb->kode_nim){$slpro='selected';$stjobnam=$jb->nama_prodi;}
                          $toperprodi=$this->M_belanja->get_TotPerProdiB($jb->kode_nim);
                          ?>

                           <option value="<?=base_url('C_rekapPenjualan/sortStatusProdi/'.$jb->kode_nim.'/1')?>" <?=$slpro?>> <?=$jb->kode_nim?> - <?=$jb->nama_prodi?>  ( Rp <?=number_format($toperprodi,0,',','.')?> ) </option>
...
?>
  ...
  if ($statusP=='R') {
    $this->load->view('pages/master_admin/rekap/pembelian/riwayat_beliPerStatus',$d_cetak);  
  }else{
    
    $this->load->view('pages/master_admin/rekap/pembelian/riwayat_beliPerAkun',$d_cetak);  
  }
  ?>
