<form method="post" id="theform">
      ...
      if ($list->num_rows()>0){
        ...
        foreach($list->result() as $t){
         ...
         /////hapus bila barang kosong
          ...
          $qty2ketranjang=$this->Mtrans->get_produkqty_dikeranjang($t->id_produk,$id_user); ///qty yang udah di pesan
  ...
  <?php
    ...
    if($this->session->userdata('login')==TRUE and $this->session->userdata('wewenang')=='user'){
         ////////////________________________________VIA VOUCHER----------------------------------->        
    ...
    <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne"><span class="glyphicon glyphicon-chevron-right"></span>
    DOMPET
    </a>
    ...
    <div id="collapseOne" class="panel-collapse collapse">
      <div class="panel-body">
            ...
            <h3 style="font-family: number;" >Saldo : Rp <?=number_format($saldo_voc,2,',','.')?></h3>
    
        <?php
       $vumy=['',''];
        if($saldo_voc > 0 and $saldo_voc >= $subtot){
          $vumy=[
          0 => '
        <label class="radio-inline">
        <input  type="radio"  name="metode" id="inlineRadio1" value="DOMPET"/> Setuju
        </label>',
          1=>'',
          ]  ;
         ...
         $vumy=[
          0 => '
        <label class="radio-inline">
        <input type="radio" disabled="disabled" /> Setuju
        </label>
        <br/>',
          1=>'<small class="text-danger">(-) Saldo Tidak cukup</small> <br />',
          ]  ;
         ...
         if($katpar!=$bar_no){
            ...
            $vumy=[
                      ...
                      <br/>',
                      1=>'
                          <small class="text-danger">(+) Kategori <u>Parsel Lebaran</u></small>
                      ',
                      ]  ;
 ...
 <span class="glyphicon glyphicon-chevron-right"></span> VIA TRANSFER
    </a>
    ...
    <div id="collapseOne2" class="panel-collapse collapse">
      <div class="panel-body">
           ...
           <label class="radio-inline">
      <input type="radio"  name="metode" id="inlineRadio1" value="TRANSFER"> <span class="text-muted">VIA BSM</span>
    </label>
    ...
    <div class="panel-heading">
      <h4 class="panel-title">
         ...
         <label class="radio-inline dckdesain">
      <input type="radio" class="dckdesain"  name="metode" id="inlineRadio2" value="UMY" disabled><span class="text-muted"> BIRO KEUANGAN UMY</span>
    </label>
...
//echo $subtot_kali;
 ?>
    ...
    if ($list->num_rows()>0 and $pos == false and $subtot_kali !=0){ ?>
      ...
      <input type="button" class="btn btn-success btn-lg btn-block" data-toggle="modal" data-target="#myModal" value="PROSES PEMESANAN" /></br>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">PROSES PEMESANAN</h4>
              </div>
              <div class="modal-body">
                <p class="h4" align="center">
                  Barang bisa diambil 2 hari setelah pemesanan yaitu pada </br>
                  Tanggal  <b><?=$this->M_time->add_hari(2)?></b>

                </p>
              </div>
              <div class="modal-footer">
                <input type="submit" id="btnsubmit" value="SETUJU" class="btn btn-success btn-block" onclick="konfirmasibayar('<?=base_url('C_pesanproduk/bayar/'.$id_pembeli)?>')">
              </div>
            </div>
          </div>
        </div>
